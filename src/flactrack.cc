...
flactrack::flactrack(const string &afname)
...
bool flactrack::terminated() const {
    return cpid == -1;
}

int flactrack::getlength() const {
    return length;
}

string flactrack::getdescription() const {
    string r;

    r = (string) "(flac) " + artist;
    if(!artist.empty()) r += ": ";
    r += title;
...
vector<string> flactrack::getstatus() const {
    vector<string> r;
	...
	if(term) {
	    ...
	    fclose(fpread);
	    fclose(fpwrite);
	    ui.status(_("The flac player application has terminated"));
	}
    }

    sprintf(cbuf, _("FLAC: %s [%02d:%02d]"),
...
void flactrack::toplaylist(ofstream &f) const {
    f << fname << endl;
}

void flactrack::readtag() {
    struct stat st;
...
void flactrack::runplayer() {
	...
	if(!(cpid = fork())) {
	    ...
	    setpgid(0, 0);
	    execl("/bin/sh", "/bin/sh", "-c", (conf.getflacplayer() + " '" + fname + "'").c_str(), 0);
		...
		cpid = -1;
		ui.status(_("Cannot run the FLAC player program"));
	    } else {
		ui.status(_("The FLAC player program has been launched"));
	    }
	}
    }
}

void flactrack::setidtags(string &aartist, string &atitle, string &aalbum, string &ayear, string &acomment) {
	...
	unlink(tfname.c_str());
	ui.statusf("Coudln't update the FLAC ID tags");
    }
#endif
}
